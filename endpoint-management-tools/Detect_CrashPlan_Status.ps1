...
function CheckCrashPlanInstall {
    ...
    if (Test-Path -Path $ServiceLogPath -PathType Leaf) {
        ...
        if (-not $Authorized) {
            ...
            $LastDeployLine = $($ServiceLog | Select-String -Pattern 'Deploy:: Retrieving deployment package' | Select-Object Line -Last 1)
            if ($null -ne $LastDeployLine) {
                $LastDeployFound = $LastDeployLine -match '\d{2}\.\d{2}\.\d{2} \d{2}:\d{2}:\d{2}'
                if ($LastDeployFound) {
                    $LastDeployString = $matches[0]
                }
                $LastDeployTime = [datetime]::ParseExact($LastDeployString, 'MM.dd.yy HH:mm:ss', [System.Globalization.CultureInfo]::GetCultureInfo('en-US'))
            }
            $LastStartLine = $($ServiceLog | Select-String -Pattern 'STARTED CrashPlan Agent' | Select-Object Line -Last 1)
            if ($null -ne $LastStartLine) {
                $LastStartFound = $LastStartLine -match '\d{2}\.\d{2}\.\d{2} \d{2}:\d{2}:\d{2}'
                if ($LastStartFound) {
                    $LastStartString = $matches[0]
                }
                $LastStartTime = [datetime]::ParseExact($LastStartString, 'MM.dd.yy HH:mm:ss', [System.Globalization.CultureInfo]::GetCultureInfo('en-US'))
            }
        }
